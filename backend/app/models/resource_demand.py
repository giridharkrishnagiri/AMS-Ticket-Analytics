...
class ResourceDemandServiceLevelSplit(UuidPrimaryKeyMixin, TimestampMixin, Base):
    __tablename__ = "resource_demand_service_level_splits"
    __table_args__ = (
        UniqueConstraint(
            "project_id",
            "ticket_type",
        ...
        Index("ix_resource_demand_service_splits_project", "project_id"),
    )
    ...
    incident_source: Mapped[str] = mapped_column(String(40), nullable=False, default="Any")
    technology: Mapped[str] = mapped_column(String(120), nullable=False, default="Generic")
    l1_5_pct: Mapped[float | None] = mapped_column(Numeric(6, 2), nullable=True)
    l2_pct: Mapped[float | None] = mapped_column(Numeric(6, 2), nullable=True)
    l3_pct: Mapped[float | None] = mapped_column(Numeric(6, 2), nullable=True)
    sort_order: Mapped[int] = mapped_column(default=0, nullable=False)
